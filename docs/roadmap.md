...
## M4 — 사용자 검증 & 커뮤니티
1차 웨지는 **한국어 학습(인강·도표)**이다. 근거: NVDA 스토어 503개 애드온 중 `ko`
번역 0개(언어 moat), 시각장애인 230명 중 학업 도표를 AI로 읽는 비율 7.5%·50%는 여전히
타인에게 물어봄(충족되지 않은 실수요). 게임은 근거가 반박(GamerAstra: VLM 전용 모드는
성공률을 못 높임)이라 **후속**으로 강등. 근거 출처는 [evidence.md](evidence.md).
- 시각장애인 사용자 테스트: **학습(인강/도표) 5명 이상 우선** + 게임 소수(탐색적).
  - 협력 타진: 한국시각장애인연합회, 넓은마을 등
- **검증 프로토콜은** [user-testing.md](user-testing.md) — 특히 오류 탐지율(planted-error)
  측정으로 "안내가 자율 대행보다 안전한가"를 실증(M7 자율성 확대의 게이트).
...
- 데모 영상 (**인강 도표·수식 이해가 메인 시나리오**, 게임은 보조)
...
## M7 — 안내 우선 코파일럿  📐 설계 재검토 완료 (범위 축소)
화면을 *읽어주는* 해설자에서, **다음 안전한 한 수를 지목해 사용자를 이끄는** 안내자로.
초안은 "대신 화면을 조작하는" 자율 대행(자비스)이었으나, 근거 검증 결과 이 사용자군은
AI 오류를 약 50%만 잡아내(감독·확인이 신뢰 불가) **자율 대행은 안전 마진이 음수**로
판단, **안내 우선 + 사용자 개시·되돌림 가능 행동에 한한 좁은 자동화**로 재정의했다.
전체 설계·근거는 [agent-copilot.md](agent-copilot.md), [evidence.md](evidence.md).
- 안내 루프: 의도 청취(STT) → 화면 이해 → **다음 한 수 지목**(위치·레이블·단축키) →
  **사용자가 실행** → 상태 보고. 실행 주체는 사람, 자동화는 사용자가 개시하는 예외.
- 그라운딩: 접근성 트리 우선(빠름·정확·비전 0). 비전 좌표는 **안내 전용**이며 좌표
  흔들림 때문에 **무인 클릭 근거로는 쓰지 않는다**(검증 불가 위험).
- 재사용(정정): `GoalTracker`(의도 세션), `NarrationEngine`(안내·상태 보고),
  `ChangeDetector`(변화 감지 → 보고 트리거), `providers/*`(두뇌), `triage`(스캠/보안
  탐지 → **자동화 제외 판단의 입력**). ※ 초안의 "`safety.apply_policy`가 행동을
  차단한다"는 **오류**였음 — 실제 함수는 `triage.apply_policy`이고 나가는 행동이 아니라
  **들어오는 이벤트**를 분류하며 차단 기능이 없다. **행동 게이트는 신규**(`agent/
  automator.py`)로 만든다. "멈춰" barge-in도 미구현이라 **신규** 항목.
- [x] M7-a: 에이전트 스캐폴드(`core/bitgil_core/agent/`: `intent`/`advisor`/
  `automator`(신규 순수 행동 게이트 `gate_action`)/`grounding`(트리 우선)/`report`/
  `loop`(`AdvisorLoop` + "멈춰" barge-in 신규 구현)) + 페이크 어댑터 오프라인 테스트
  23개(`tests/test_agent.py`). 게이트는 **기본 차단**: 사용자 개시 + 되돌림 가능 +
  트리 식별 + 비민감 대상일 때만 자동화 허용, 그 외는 안내로 강등.
- [ ] M7-b: 음성 의도(STT) + 브라우저 **안내** 루프 → 실동작 데모(자동화 아님, 안내부터).
- [ ] M7-c: 좁은 자동화(사용자 개시·되돌림 가능·트리 식별) — 읽기 전용 내비게이션부터 +
  "멈춰" 중단 제어 신규 구현.
- [ ] M7-d: Windows UIA 어댑터 (실기기). 자율성 확대는 사용자 테스트 실증 후에만.
