...
- 같은 이유로 모델이 답을 구조화하려고 줄 앞에 붙인 목록 번호("5) 전반적인 추세")도
  검사에서 뺍니다.
- **기간을 센 말("4년간의 증가 추세")도 뺍니다.** 화면의 라벨을 읽었다는 주장이 아니라
  라벨을 센 결과입니다("2023년"처럼 라벨 그 자체인 표기는 그대로 검사합니다).
"""
...
#
# "년간/개월간"은 실측에서 나온 오탐입니다(Bedrock, 2026-08-27): 2021~2024년 막대그래프
# 설명이 "4년간의 꾸준한 증가"로 끝나자 4가 근거 없는 숫자로 고지됐습니다. 이건 라벨을
# 읽었다는 주장이 아니라 라벨을 센 결과입니다. 라벨 자체인 "2023년"은 계속 검사합니다 —
# 축 눈금을 지어내는 것이야말로 이 검사가 잡아야 하는 것이기 때문입니다.
_NUMBER = re.compile(
	r"(\d+(?:,\d{3})*(?:\.\d+)?)\s*(번째|번|쪽|페이지|째|개월간|년간)?"
)
